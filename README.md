...
## התאמת מסך הבית למובייל ללא גלילה

הריווח (padding/gap), גדלי הכותרת/תת-כותרת וכרטיסי הקטגוריה ב-`HomePageTrivia.css` (השכבה הבסיסית, מתחת ל-700px) כווננו כך שכל מסך הבית ייכנס במסך אחד ב-iPhone 16 Pro Max בלי גלילה. **אילוץ חשוב:** כפתור ההגדרות (`#settingsButton`) ותג השחקן (`#homePlayerBadgeContainer`) הם `position: fixed; top: 16px`, ומכיוון ש-`.app-title` מיושר לשמאל (`text-align: left`) הוא נכנס בדיוק מתחת לאייקון גלגל השיניים — לכן ל-`margin-top` של הכותרת יש רצפה בטוחה (כרגע 40px) שאסור להקטין בלי לבדוק חפיפה מול האייקון הקבוע, לא רק את גובה התוכן הכולל. השכבות `@media (min-width: 700px/1000px)` (טאבלט) לא הושפעו — יש להן override נפרד ל-`margin-top`.

**חשוב לבדיקה:** פתיחת DevTools עם F12 בלבד (בלי Device Toolbar / Ctrl+Shift+M) מציגה את גובה חלון המחשב האמיתי (הרבה יותר גבוה ממסך טלפון), ומכיוון ש-`.home-screen` משתמש ב-`justify-content: center`, זה יוצר רווח ריק גדול מעל ומתחת לתוכן — זו התנהגות מכוונת (עיצוב "פאנל" ממורכז על רקע רחב), לא באג. כדי לבדוק תצוגת מובייל אמיתית יש להפעיל את Device Toolbar ולבחור את המכשיר המתאים.
